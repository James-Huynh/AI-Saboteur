...
public class MyTools {
	 ...
	 * heuristic based approach. In this case, the heuristic is the length of a path
	 * to an objective.
	 * 
	 * @param arrLegalMoves is an array containing all legal moves from the current
	 *                      player
	...
	// James
	/**
	 * This methods chooses which objective an AI should prioritize. The choice is
	 * determined by which objective is revealed, and if the gold nugget is
	 * revealed.
	 * 
	 * @param boardState is the boardState of the current turn
	...
	private static int[] targetObjective(SaboteurBoardState boardState) {
		...
		SaboteurTile[] objectives = { hiddenBoard[coordObj1[0]][coordObj1[1]], hiddenBoard[coordObj2[0]][coordObj2[1]],
	...
	private static boolean isWinningMove(SaboteurMove move, int[] coordinates) {
		...
		if (posPlayed[0] < targetPos[0]) {
			c = 'n';
		} // north
		if (posPlayed[0] > targetPos[0]) {
			c = 's';
		} // south
		if (posPlayed[1] < targetPos[1]) {
			c = 'w';
		} // west
		if (posPlayed[1] > targetPos[1]) {
			c = 'e';
	...
	// Tugu
	/**
	 * checks for an existence of a dead end card in the surrounding 4 tiles around
	 * the passed saboteurMove's coordinate
	 * 
	...
	private static boolean isFakePath(SaboteurBoardState boardState, SaboteurMove move) {
		...
		if (i < 14 && i >= 0 && j < 14 && j >= 0 && board[i][j] != null) {
			...
			if (tileIdx.equals("1") || tileIdx.equals("2") || tileIdx.equals("2_flip") || tileIdx.equals("3")
					|| tileIdx.equals("3_flip") || tileIdx.equals("4") || tileIdx.equals("4_flip")
					|| tileIdx.equals("11") || tileIdx.equals("11_flip") || tileIdx.equals("12")
					|| tileIdx.equals("12_flip") || tileIdx.equals("13") || tileIdx.equals("13_flip")
					|| tileIdx.equals("14") || tileIdx.equals("14_flip") || tileIdx.equals("15")
					|| tileIdx.equals("15_flip")) {
				retVal = true;
			}
		}
		i = x + 1;
